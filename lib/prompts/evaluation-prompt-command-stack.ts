...
[C2.5]: 🚨🚨🚨 最優先事項：エビデンス必須表現の検証 🚨🚨🚨

**【THIS IS THE MOST IMPORTANT RULE - 他のすべてのルールより優先】**

セグメント評価を開始する前に、以下の表現が含まれているか必ずチェックしてください：

### ✅ 必須チェック項目

**1. ランキング・順位表現のチェック**
- 含まれている表現: NO.1 / ナンバーワン / 第1位 / トップ / Amazonランキング / 楽天ランキング / Amazon・楽天で1位 / 売上NO.1 / 人気NO.1 / 満足度NO.1
- ✅ エビデンス明示あり（※注釈+出典） → OK
- ❌ エビデンス明示なし → **景表法違反（重大）として必ず報告**

**2. リピート率表現のチェック**
- 含まれている表現: リピート率○% / ○%の方がリピート / 継続率○%
- ✅ 算出方法の注釈あり → OK
- ❌ 注釈なし → **景表法違反（重大）として必ず報告**

**3. 販売実績表現のチェック**
- 含まれている表現: 累計○個突破 / 累計販売○個 / ○秒に○個売れている / ○万人が愛用
- ✅ 集計期間の注釈あり → OK
- ❌ 注釈なし → **景表法違反（重大）として必ず報告**

**4. 世界初・日本初表現のチェック**
- 含まれている表現: 世界初 / 日本初 / 業界初 / 唯一 / 独自
- ✅ 調査機関・調査日の注釈あり → OK
- ❌ 注釈なし → **景表法違反（重大）として必ず報告**
...
❌ 「Amazon・楽天で1位を獲得した人気商品です。」
   → 理由: ランキング表現に注釈（※）とエビデンスがない
   → 判定: 景表法違反（重大）- エビデンス明示義務違反
   → severity: "critical"
   → description: "「Amazon・楽天で1位」というランキング表現にエビデンス（注釈※と出典情報）が明示されていません。景表法により、ランキング・順位の表現には調査機関・調査期間・調査対象の明記が必須です。"

**OK例:**
✅ 「Amazon・楽天で1位※1を獲得した人気商品です。」
   ※1 2024年1月楽天市場・Amazon売上ランキング1位
   → 理由: 注釈でエビデンスを明示している

### ⚠️ 重要な注意事項

1. **知識ベースに記載がなくても、上記表現があれば必ずチェックすること**
2. **「例として記載」されていても、エビデンスがなければ違反**
3. **この検証は他のすべての評価より前に実施すること**
4. **1つでも該当する表現があれば、必ず違反として報告すること**
5. **このルールは[C8]の「知識ベースに根拠がない場合は適合」より優先される**
...
4. **最上級表現・エビデンス必須**: 「No.1」「ナンバーワン」「第1位」「トップ」「業界初」「世界初」「Amazon1位」「楽天1位」「ランキング1位」
   - ⚠️ これらの表現は**エビデンス明示が必須**です（詳細は[C2.5]参照）
5. **期間限定**: 「今なら」「今だけ」「期間限定」「本日限り」
6. **専用表現**: 「〇〇専用」「〇〇向け」
7. **販売実績・エビデンス必須**: 「リピート率○%」「累計○個」「○秒に○個売れている」
   - ⚠️ これらの表現は**エビデンス明示が必須**です（詳細は[C2.5]参照）
...
**【例外】[C2.5]エビデンス必須表現の優先適用:**
ただし、[C2.5]で定義されたエビデンス必須表現（ランキング・順位、リピート率、販売実績、世界初・日本初等）については、
**知識ベースの記載有無に関わらず**、エビデンスが明示されていない場合は必ず景表法違反（重大）として検出してください。
[C2.5]の指示は他のすべての指示よりも優先されます。
